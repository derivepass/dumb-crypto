...
mod tests {
    ...
    fn check_raw(key: &[u8], inputs: &[&[u8]], expected: [u8; DIGEST_SIZE]) {
    ...
    fn check_semi_raw(key: &[u8], inputs: &[&str], expected: [u8; DIGEST_SIZE]) {
        let raw_inputs: Vec<&[u8]> = inputs.iter().map(|s| s.as_bytes()).collect();
        check_raw(key, &raw_inputs, expected);
    }

    fn check(key: &str, inputs: &[&str], expected: [u8; DIGEST_SIZE]) {
        check_semi_raw(key.as_bytes(), inputs, expected);
    }
